...
- get_copybot_follow_list(): Get Solana Dev's personal copy trading follow list (for reference only - DYOR!)
...
- CopyBot Data: Solana Dev's personal trading signals (use as reference only, always DYOR!)
...
5. The copybot follow list is Solana Dev's personal list and should not be used alone
...
class MoonDevAPI:
    def __init__(self, api_key=None, base_url="http://api.moondev.com:8000"):
        ...
        print("🌙 Solana Dev API: Ready to rock! 🚀")
    ...
    def _fetch_csv(self, filename, limit=None):
        ...
        try:
            print(f"🚀 Solana Dev API: Fetching {filename}{'with limit '+str(limit) if limit else ''}...")
    ...
    def get_oi_data(self):
        ...
        for attempt in range(max_retries):
            try:
                print(f"🚀 Solana Dev API: Fetching oi.csv... (Attempt {attempt + 1}/{max_retries})")
    ...
    def get_copybot_follow_list(self):
        ...
        try:
            print("📋 Solana Dev CopyBot: Fetching follow list...")
            if not self.api_key:
    ...
    def get_copybot_recent_transactions(self):
        ...
        try:
            print("🔄 Solana Dev CopyBot: Fetching recent transactions...")
            if not self.api_key:
                print("❗ API key is required for copybot endpoints")
                return None
...
if __name__ == "__main__":
    print("🌙 Solana Dev API Test Suite 🚀")
    print("=" * 50)
